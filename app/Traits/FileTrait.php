...
trait FileTrait
{
     ...
     * @return $this|false|string
     */
    public function storeFile(Request $request, $fieldname = 'image', $directory = 'files')
    {
        if ($request->hasFile($fieldname)) {
            ...
            $path = $request->file($fieldname)->storeAs("$directory", $filenameToStore, 'public');
    ...
    public function editFile($oldFilename, Request $request, $fieldname = 'image', $directory = 'files')
    {
        if ($request->hasFile($fieldname)) {
            if ($this->deleteFile($oldFilename, $directory)) {
                return $this->storeFile($request, $fieldname, $directory);
            }
        }
        return $oldFilename;
    }
